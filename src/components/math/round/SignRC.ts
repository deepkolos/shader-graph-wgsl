...
export type ReteSignNode = ExtendReteNode<
  'Sign',
  {
...
export class SignRC extends RC {
  constructor() {
    super('Sign');
    this.data.component = NodeView;
  }

  initNode(node: ReteSignNode) {
  ...
  async builder(node: ReteSignNode) {
    this.initNode(node);
  ...
  compileSG(compiler: ShaderGraphCompiler, node: SGNodeData<ReteSignNode>): SGNodeOutput {
    const outVar = compiler.getOutVarName(node, 'out', 'sign');
