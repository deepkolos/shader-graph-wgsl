...
export type ReteNegateNode = ExtendReteNode<
  'Negate',
  {
...
export class NegateRC extends RC {
  constructor() {
    super('Negate');
    this.data.component = NodeView;
  }

  initNode(node: ReteNegateNode) {
  ...
  async builder(node: ReteNegateNode) {
    this.initNode(node);
  ...
  compileSG(compiler: ShaderGraphCompiler, node: SGNodeData<ReteNegateNode>): SGNodeOutput {
    const outVar = compiler.getOutVarName(node, 'out', 'neg');
