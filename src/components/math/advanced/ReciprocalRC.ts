...
export type ReteReciprocalNode = ExtendReteNode<
  'Reciprocal',
  {
...
export class ReciprocalRC extends RC {
  constructor() {
    super('Reciprocal');
    this.data.component = NodeView;
  }

  initNode(node: ReteReciprocalNode) {
  ...
  async builder(node: ReteReciprocalNode) {
    this.initNode(node);
  ...
  compileSG(compiler: ShaderGraphCompiler, node: SGNodeData<ReteReciprocalNode>): SGNodeOutput {
    const outVar = compiler.getOutVarName(node, 'out', 'rec');
