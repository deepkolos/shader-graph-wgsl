...
export type ReteLogNode = ExtendReteNode<
  'Log',
  {
...
export class LogRC extends RC {
  constructor() {
    super('Log');
    this.data.component = NodeView;
  }

  initNode(node: ReteLogNode) {
  ...
  async builder(node: ReteLogNode) {
    this.initNode(node);
  ...
  compileSG(compiler: ShaderGraphCompiler, node: SGNodeData<ReteLogNode>): SGNodeOutput {
    const outVar = compiler.getOutVarName(node, 'out', 'log');
