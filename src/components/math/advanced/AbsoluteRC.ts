...
export type ReteAbsoluteNode = ExtendReteNode<
  'Absolute',
  {
...
export class AbsoluteRC extends RC {
  constructor() {
    super('Absolute');
    this.data.component = NodeView;
  }

  initNode(node: ReteAbsoluteNode) {
  ...
  async builder(node: ReteAbsoluteNode) {
    this.initNode(node);
  ...
  compileSG(compiler: ShaderGraphCompiler, node: SGNodeData<ReteAbsoluteNode>): SGNodeOutput {
    const outVar = compiler.getOutVarName(node, 'out', 'abs');
